/*
Zadanie na pracownię numer 6 - Grafika Komputerowa
...
void Cube :: Draw(){ 
    ...
    //ściany sześcianu i ich normalne - znalezione w Internecie
    glBegin(GL_QUADS);
...
class AABB : public BoundingVolume{
    ...
    AABB(Cube* c){ //można stworzyć bryłę otaczającą na podstawie sześcianu
        type = BBOX;
...
/************************************************************************************/
//klasa abstrakcyjna dla obiektów, które mogą się poruszać
//tak naprawdę okazała się niepotrzebna, ale nie reorganizowałem ponownie struktury kodu
class Moveable{
    protected:
    Point position;
    Point moveDirection; //aktualny kierunek ruchu
    //aktualna prędkość, przyspieszenie, prędkość maksymalna
    double speed, acceleration, max_speed;
    
    //wykonuje ruch obiektu. Włącznie z odbiciem od przeszkód
    //dir - żądany kierunek
    //time - jak długo obiekt poruszał się w tym kierunku
    virtual void Move(Point dir, double time, Octree* bsp){
        //obiekt nie wykonał ruchu
        //następuje wytracenie szybkości
        if(dir.x == 0 && dir.y == 0 && dir.z == 0){
            //tu i niżej mamy do czynienia z implementacją pseudobezwładności
            //nie wiem ile ma ona wspólnego z równaniami fizycznymi
            //ale w grze sprawdza się wyśmienicie
            speed -= (acceleration / 2) * time;
            speed = speed > 0 ? speed : 0;
        }
        else{
            //oblicz nowy kierunek ruchu i prędkość
        ...
        //sprawdzam czy obiekt koliduje z jakąś przeszkodą
        //jeśli tak to odbijam go w najprostszy z możliwych sposobów -
        //zmieniając zwrot jego kierunku ruchu
        ...
        position += time * speed * moveDirection; // właściwy ruch
    }
};
//latający po planszy przeciwnicy
class Enemy : public Moveable{
    private:
    //time pełni rolę licznika - gdy jego wartość spadnie do zera
    //obiekt przechodzi w nowy stan
    double time;
    Cube model;
    Point dir;
    ...
    Enemy(Point pos){
        ...
        time = 0;
        //zielony przeciwnik przemierza planszę losowo
...
bool Enemy :: Update(double diff, Octree* bsp, Point player){
    float scope = 50; //zasięg "radaru" przeciwnika
    AABB bounding(Point(1, 1, 1) + position, Point(-1, -1, -1) + position); //AABB przeciwnika
    AABB radar(scope * Point(1, 1, 1) + position, scope * Point(-1, -1, -1) + position); //AABB radaru
    AABB camera(Point(1, 1, 1) + player, Point(-1, -1, -1) + player); //AABB gracza
    if(bounding.Intersects(&camera)){
        return true; //przeciwnik dopadł gracza
    }
    if(radar.Intersects(&camera)){ //przeciwnik namierzył gracza
        if(model.color.x == 0){ //zielony -> żółty
            model.color = Point(1, 1, 0);
            dir = Point(0, 0, 0);
            time = 0.6;
        }
        if(model.color.x == 1 && model.color.y == 1 && time <= 0){ //żółty -> czerwony
            model.color = Point(1, 0, 0);
            time = 3;
        }
        if(model.color.y == 0){ //czerwony
            dir = (player + (-1) * position); //kieruj się na przeciwnika
            dir = dir.Normalize();
            max_speed = 75;
        }
        
    }
    if(time <= 0){ //przejście z dowolnego koloru w zielony
        time = 6;
        //wyznaczamy nowy, losowy kierunek ruchu
...
//generuje przeciwników na losowych pozycjach
...
Camera camera;
Cube finish; //meta
//ściany ograniczające całą planszę
Cube top, bottom, leftC, rightC, front, back;
void SetUp(){ //ustaw wszystkie potrzebne zmienne

    //drzewo bsp + przeszkody
...
void CleanUp(){ //jak sama nazwa wskazuje
    for(list<Cube*> :: iterator it = cubes -> begin(); it != cubes -> end(); it++){
        delete *it;
    }
...
int main( void ){
    ...
    GLfloat LightPosition[]= { 0, 2, -2, 1.0f };
    ...
    while(running){        
        ...
        //sprawdzenie czy gracz nie wygrał
        AABB fin(&finish);
        ...
        //update + sprawdzenie przegranej gracza
        ...
        for(set<Cube*> :: iterator it = toDraw -> begin(); it != toDraw -> end(); it++){
            (*it) -> Draw(); //wyświelta przeszkody
        }
        delete frustum;
        for(list<Enemy*> :: iterator it = enemies -> begin(); it != enemies -> end(); it++){
            (*it) -> Draw(); //wyświetla przeciwników
        }
        finish.Draw(); //wyświetla metę
