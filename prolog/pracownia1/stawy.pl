%Stawy i groble - zadanie nr 6
%Przemysław Pastuszka 2010
%
%Algorytm rozwiązywania zagadki działa następująco:
%-z każdej komórki, która zawiera cyfrę, zainicjuj dfs (iWannaLake) konstruujący staw
%	-gdy pomijasz komórkę oznacz ją jako groblę (zapobiega to stykaniu się stawów bokami
%		oraz zabezpiecza przed kilkukrotnym sprawdzaniem tego samego ustawienia)
%	-jeśli staw sąsiaduje z dwoma polami, które są oznaczoną (!) groblą, sprawdź czy istnieje
%		ścieżka między tymi polami (ta heurystyka ma na celu szybkie odrzucanie ustawień
%		"złych", tj. kiedy doszło do rozspójnienia grobli)
%-na koniec sprawdź spójność grobli i jej grubość
%
...
main(File) :-
   ...
   compute(LakeInitCells, Board, LeeveSize), %oblicz rozwiązanie
...
init(InitData, cell((X, Y), _, NR, W)) :-
	...
	NR is Q - 1, %rozmiar stawu pomniejszamy o jeden, gdyż pierwsza komórka została już oznaczona jako jego część
	W is X * Y,  !.
init(_, _).
...
iWannaLake([cell(_, Nghbs, _, X) | T], HowMany, LakeNr) :-
	...
	append(Nghbs, T, MoreNghbs), %sąsiedzi stają się potencjalnymi kandydatami na dalsze części stawu
...
coverWithLeeve([], _) :- !.
coverWithLeeve([cell(_, _, _, leeve)|T], LakeNr) :-
	!, 
	coverWithLeeve(T, LakeNr).
coverWithLeeve([cell(_, _, _, LakeNr)|T], LakeNr) :-
	coverWithLeeve(T, LakeNr).

compute([], FullBoard, LeeveSize) :- !,
	findLeeve(FullBoard, Leeve), %po stworzeniu stawów sprawdzamy spójność grobli 
	isLeeveOK([(1, Leeve)], LeeveSize).
compute([cell(_, Nghbs, HowMany, Nr)|Tail], FullBoard, LeeveSize) :-
   iWannaLake(Nghbs, HowMany, Nr), %wywołanie dfs'a tworzącego staw
   compute(Tail, FullBoard, LeeveSize).
...
%sprawdzenie czy grobla jest spójna (i odpowiedniej grubości)
%algorytm banalny - policz komórki do których możesz dojść idąc po grobli
%ich ilość powinna równać się żądanemu rozmiarowi grobli (czyli szerokość*wysokość - powierzchnia stawów)
%ponadto sprawdzamy czy grubość grobli jest poprawna - zgłaszamy fail, gdy wykryliśmy cykl 4-elementowy na grobli
isLeeveOK(_, 0) :- !. 
isLeeveOK([], _) :-  !, fail.
isLeeveOK([(Nr, cell(_, Nghbs, X, leeve))|T], N) :-
	var(X),
	!,
	X = Nr,
	NN is N - 1,
	NNr is Nr + 1,
	specialAppend(Nghbs, T, MoreNghbs, NNr),
	isLeeveOK(MoreNghbs, NN).
isLeeveOK([(A, cell(_, _, B, leeve))|_], _) :-
	4 is A - B,
	!,
	fail.
...
specialAppend([], Y, Y, _).
specialAppend([H|T], Y, [(Nr, H)|S], Nr) :-
	specialAppend(T, Y, S, Nr).
