...
ifEval(bool(true), Fst, _, (Env, NewEnv), (N, NewN), R) :- !, evaluate(Fst, (Env, NewEnv), (N, NewN), R).
ifEval(bool(false), _, Snd, (Env, NewEnv), (N, NewN), R) :- !, evaluate(Snd, (Env, NewEnv), (N, NewN), R).
...
evaluate(seq(fun(Name, Body), Rest), (Env, NewEnv), (N, NewN), R) :- 
	!,
	evaluate(Body, (Env, Env1), (N, N1), BR),
	substitute(BR, Name, id(N1), NewBody),
	substitute(Rest, Name, id(N1), NewRest),
	N2 is N1 + 1,
	evaluate(NewRest, ([(N1, NewBody)|Env1], NewEnv), (N2, NewN), R).
evaluate(con(Condition, Fst, Snd), (Env, NewEnv), (N, NewN), R) :- 
	!, 
	evaluate(Condition, (Env, Env1), (N, N1), B), 
	ifEval(B, Fst, Snd, (Env1, NewEnv), (N1, NewN), R).
evaluate(lambda(V, B), (Env, Env), (N, N), lambda(V, B)) :- !.
evaluate(num(X), (Env, Env), (N, N), num(X)) :- !.
evaluate(list(X), (Env, Env), (N, N), list(X)) :- !.
evaluate(bool(X), (Env, Env), (N, N), bool(X)) :- !.
evaluate(prefix(X), (Env, Env), (N, N), prefix(X)) :- !.
evaluate(id(X), (Env, Env), (N, N), R) :- !, get(Env, X, R).
evaluate(binary(X, Y, Z), (Env, NewEnv), (N, NewN), R) :- !, 
	evaluate(X, (Env, Env1), (N, N1), XR), 
	evaluate(Z, (Env1, NewEnv), (N1, NewN), ZR), 
	evalBin(Y, XR, ZR, R).
evaluate(appl(prefix(X), Y), (Env, NewEnv), (N, NewN), R) :- !,
	evaluate(Y, (Env, NewEnv), (N, NewN), YR),
	evalPref(X, YR, R).
evaluate(appl(id(X), Y), (Env, NewEnv), (N, NewN), R) :- !,
	get(Env, X, XR), evaluate(appl(XR, Y), (Env, NewEnv), (N, NewN), R).
evaluate(appl(appl(A, B), C), (Env, NewEnv), (N, NewN), R) :- !,
	evaluate(appl(A, B), (Env, Env1), (N, N1), ABR),
	evaluate(appl(ABR, C), (Env1, NewEnv), (N1, NewN), R).
evaluate(appl(lambda(V, B), X), (Env, NewEnv), (N, NewN), R) :- !,
	evaluate(X, (Env, Env1), (N, N1), XR),
	substitute(B, V, XR, BR),
	evaluate(BR, (Env1, NewEnv), (N1, NewN), R).
