...
fix(F) :-
	copy_term(
	lambda(X, appl(lambda(Y, lambda(Z, appl(appl(X, appl(Y, Y)), Z))), lambda(W, lambda(Q, appl(appl(X, appl(W, W)), Q))))),
	F).

evaluate(seq(fun(Name, B), Rest), Env, R) :-
	!,
	evaluate(B, Env,  NBody),
...
evaluate(lambda(V, B), _, lambda(V, B)) :- !.
	%renameFV(lambda(V, B), Env, lambda(V, C)).
...
evaluate(appl(id(X), Y), Env, R) :- !,
	evaluate(id(X), Env, XR),
	evaluate(appl(XR, Y), Env, R).
evaluate(appl(appl(A, B), C), Env, R) :- !,
	evaluate(appl(A, B), Env, ABR),
	evaluate(appl(ABR, C), Env, R).
evaluate(appl(lambda(X, B), X), Env, R) :- !,
	evaluate(B, Env,  R).
evaluate(appl(A, B), _, _) :- !,
	write('Error: wrong application: '), write(A), write(' applied to '), write(B), fail.
evaluate(Q, _, _) :-
...
preprocessFun(fun(A, lambda(B, C)), Env, fun(A, appl(F, X))) :- !,
	fix(F),
	preprocess(lambda(A, lambda(B, C)), Env, X).
preprocessFun(fun(A, B), Env, fun(A, Q)) :-
	preprocess(B, Env, Q).

preprocess(lambda(A, B), Env, lambda(X, Q)) :- !,
	preprocess(B, [(A, X) | Env], Q).
preprocess(num(X), _, num(X)) :- !.
preprocess(list(X), _, list(X)) :- !.
preprocess(bool(X), _, bool(X)) :- !.
preprocess(id(A), Env, X) :-
	member((A, X), Env), !.
preprocess(id(A), _, id(A)) :- !.
preprocess(appl(A, B), Env, appl(X, Y)) :- !,
	preprocess(A, Env, X),
	preprocess(B, Env, Y).
preprocess(seq(A, B), Env, seq(X, Y)) :- !,
	preprocess(A, Env, X),
	preprocess(B, Env, Y).
preprocess(fun(A, B), Env, R) :-
	member((A, _), Env), !,
	getRidOf(Env, A, NEnv),
	preprocessFun(fun(A, B), NEnv, R).
preprocess(fun(A, B), Env, R) :- !,
	preprocessFun(fun(A, B), Env, R).
preprocess(binary(A, Q, B), Env, binary(X, Q, Y)) :- !,
	preprocess(A, Env, X),
	preprocess(B, Env, Y).
preprocess(prefix(X), _, prefix(X)) :- !.
preprocess(con(A, B, C), Env, con(R1, R2, R3)) :- !,
	preprocess(A, Env, R1),
	preprocess(B, Env, R2),
	preprocess(C, Env, R3).
