...
public class TreeFactory extends AbstractCandidateFactory<Node>
{
  ...
  // The maximum depth of a program tree. No function nodes will be created
  // below
   ...
   *          The number of program parameters that each generated program tree
   *          can will be provided when executed.
   * @param maxDepth
   *          The maximum depth of generated trees.
   * @param functionProbability
   *          The probability (between 0 and 1) that a randomly-generated node
   *          will be a function node rather than a value (parameter or
   *          constant) node.
   * @param parameterProbability
   *          The probability that a randomly-generated non-function node will
   *          be a parameter node rather than a constant node.
   */
  ...
  private Node makeNode(Random rng, int maxDepth)
  {
    if (functionProbability.nextEvent(rng) && maxDepth > 1)
    {
      ...
      switch (rng.nextInt(6))
      {
        ...
        case 3:
          return new IfThenElse(makeNode(rng, depth), makeNode(rng, depth), makeNode(rng, depth));
        case 4:
          return new MinOperator(makeNode(rng, depth), makeNode(rng, depth));
        default:
